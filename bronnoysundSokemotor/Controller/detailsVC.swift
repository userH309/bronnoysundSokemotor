...
class detailsVC: UIViewController,UINavigationBarDelegate {
    ...
    //Update UI as soon as the detailsVC loads.
    override func viewDidLoad() {
        super.viewDidLoad()
        updateUI()
    }
    
    //Set all the labels and links using data from cache.
    func updateUI() {
    ...
    @IBAction func openMapsTapped(_ sender: UIButton) {
        if mapURL != nil {
            if UIApplication.shared.canOpenURL(mapURL!) {
                UIApplication.shared.open(mapURL!, options: [:], completionHandler: { (success) in
